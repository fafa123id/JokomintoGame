...
class EventHandler {
  ...
  safeSubscribe(observable$, handler, context = "Unknown") {
    ...
    const trackingId = eventDebugger.startPerformanceTracking(
      `Subscribe_${context}`
    );
  ...
  setupNavigationEvents() {
    try {
      ...
      if (startGameBtn) {
        ...
        this.safeSubscribe(startGame$, () => sendEvent("START"), "StartGame");
      }
      ...
      if (homeButtons.length > 0) {
        ...
        this.safeSubscribe(homePage$, () => sendEvent("BACK"), "HomePage");
      }
      ...
      if (gameMenuBtn) {
        ...
        this.safeSubscribe(gameMenu$, () => sendEvent("PLAY"), "GameMenu");
      }
    } catch (error) {
  ...
  setupGameEvents() {
    ...
    // Tebak Gambar Stream
    const tebakGambarBtn = document.querySelector(
      '[data-action="tebak-gambar"]'
    );
    if (tebakGambarBtn) {
    ...
    try {
      ...
      if (soapButtons.length > 0) {
        const soapPurchases$ = merge(
          ...Array.from(soapButtons).map((btn) =>
            fromEvent(btn, "click").pipe(
              map((event) => ({
                type: "soap",
                soapType:
                  event.target.closest("[data-soap-type]").dataset.soapType,
                button: event.target,
              }))
            )
          )
          ...
          debounceTime(300),
          filter(
            ({ soapType }) => soapType && typeof window.buySabun === "function"
          )
        );

        this.safeSubscribe(
          soapPurchases$,
          ({ soapType }) => buySabun(soapType),
          "SoapPurchase"
        );
      }
      ...
      if (foodButtons.length > 0) {
        const foodPurchases$ = merge(
          ...Array.from(foodButtons).map((btn) =>
            fromEvent(btn, "click").pipe(
              map((event) => ({
                type: "food",
                foodType:
                  event.target.closest("[data-food-type]").dataset.foodType,
                button: event.target,
              }))
            )
          )
          ...
          debounceTime(300),
          filter(
            ({ foodType }) => foodType && typeof window.buyFood === "function"
          )
        );

        this.safeSubscribe(
          foodPurchases$,
          ({ foodType }) => buyFood(foodType),
          "FoodPurchase"
        );
      }
    } catch (error) {
  ...
  logPerformance(context, startTime) {
    ...
    if (duration > 100) {
      ...
      console.warn(
        `⚡ Slow operation detected in ${context}: ${duration.toFixed(2)}ms`
      );
    }
  }
...
export function initializeEventHandler() {
  try {
    ...
    if (document.readyState === "loading") {
      console.warn(
        "⚠️ DOM not fully loaded, deferring event handler initialization"
      );
      return null;
    }
    ...
    showMessage(
      "Failed to initialize event system. Some features may not work.",
      "error",
      3000
    );
    return null;
